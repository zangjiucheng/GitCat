---
description: Author a GitCat plugin — a declarative plugin.json that contributes ⌘K commands, UI panels, and lifecycle hooks backed by your own external commands.
---

# Writing a plugin

A GitCat plugin is a small, **declarative** manifest — a single `plugin.json` file. It doesn't ship code that runs inside GitCat; it contributes three things:

- **Commands** — actions that show up in the ⌘K command palette and run an external command you specify.
- **Panels** — declarative sidebar/UI widgets (headings, text, buttons, command outputs) rendered directly by GitCat.
  ...
  "panels": [
    {
      ...
      "items": [
        ...
        { "type": "text", "text": "Run checks or trigger review tools directly from GitCat." },
        { "type": "button", "label": "Run Linter", "command": "lint" },
        { "type": "command_output", "command": "lint", "label": "Latest Lint Output" }
      ]
    }
  ],
  "hooks": [
...
| commands | — | array | Zero or more commands. Defaults to []. |
| panels | — | array | Zero or more panels. Defaults to []. |
| hooks | — | array | Zero or more hooks. Defaults to []. |
| tama | — | object | Optional Tama skin — an alternate look and voice for the mascot (poses, a greeting, a voice pitch). |
| lua | — | string | Optional path (relative to the plugin folder) to a main Luau script — required only if any command/hook uses a handler. See Scripting with Luau. |
...
| run | ✳️ | The external command template. Declare exactly one of run or handler. |
| handler | ✳️ | The name of a Luau handler function instead of a shell run. Requires the plugin to declare a top-level lua script. Declare exactly one of run or handler. |
| context | — | What selection the command needs. Default "none". |
| placement | — | Where the command surfaces. Default "palette". |
| mutates | — | Set true if the command changes the repository. Default false. See Mutating actions & Undo. |

### context

context declares what the command operates on, which determines which placeholders get filled in:
...
Each entry in `panels[]` contributes a UI widget panel rendered directly inside GitCat.

### Panel structure

| Field | Required | Type | Notes |
|-------|----------|------|-------|
| id | ✅ | string | Unique identifier within the plugin. Must match `^[a-z0-9][a-z0-9-]*$`. |
| title | ✅ | string | Non-empty display title shown at the top of the panel. |
| items | ✅ | array | An array of widget items rendered sequentially inside the panel. |

### Supported widget item types

Each item in `items[]` is discriminated by its `type` field:

**Heading** (`type: "heading"`)
- `text` (string, required): Non-empty section header text.

**Text** (`type: "text"`)
- `text` (string, required): Non-empty descriptive body text.

**Button** (`type: "button"`)
- `label` (string, required): Non-empty button text.
- `command` (string, required): The id of a command declared within this same plugin.

**Command Output** (`type: "command_output"`)
- `command` (string, required): The id of a command declared within this same plugin whose output will be rendered in the panel.
- `label` (string, optional): Non-empty custom header label for the output block.

**Validation Rule:** Any command referenced by a button or command_output must exist within the same plugin's commands array. Referencing a non-existent or external command will fail manifest validation upon installation.
...
Each entry in `hooks[]` runs a run command (same template grammar and execution as a command) — or, instead, a Luau handler (see Scripting with Luau) — when a lifecycle event fires. Just like a command, a hook must declare exactly one of run or handler.
...
- **A hook can set "mutates": true** (same as a command) if it changes the repo — see Mutating actions & Undo.
...
A run string is a single shell command. When you need real logic — branch on git output, loop over files, build a message — a command or hook can instead run a handler function written in Luau (a fast, safe Lua dialect) inside a locked-down sandbox. This is still AI-agnostic and offline: the script's only reach outward is a `git()` function.

### The two manifest fields

- **lua** (top-level) — a path, relative to the plugin folder, to your main script (e.g. `"lua": "main.lua"`). The file must be a real .lua file inside the plugin folder (no `..`, no symlink escaping the folder), at most 256 KB.

- **handler** (on a command or hook) — the name of a function your script exports. A command/hook declares exactly one of run (shell) or handler (Luau); declaring both, or neither, is rejected at install. Any handler requires the plugin to declare a lua script.
...
Your main file returns a table mapping handler names to functions. Each handler is called with a single ctx argument:
...
A handler sees exactly four things (nothing else — see the sandbox):

| Name | What it is |
|------|-----------|
| ctx | A read-only table mirroring the placeholders: ctx.repo, ctx.sha, ctx.file, ctx.files (an array), ctx.diff, ctx.branch, ctx.ref. Absent fields are nil. (Populated from the UI exactly like the shell tokens — today ctx.repo always, plus ctx.sha for a commit-context command.) Also passed as the handler's argument. |
...
| tama.react(kind, msg) | Nudge Tama's mood. kind must be one of info, busy, ok, problem (the same safe allowlist as the `::gitcat.tama` stdout protocol) — any other value is ignored. A script can never reach a safety-critical pose. |
...
The command's output is everything printed, plus any tama.react lines, plus (if the handler returns a string) that string.

### The Luau sandbox {#the-luau-sandbox}

Handlers run in a fresh, isolated Luau VM built for untrusted code:

- **Only safe libraries are loaded** — string, math, table, and Luau's base library (print, pairs, pcall, tostring, …). There is no os, no io, no network, and no require / load / loadstring / dofile — a script cannot touch the filesystem, spawn processes (except through git()), open sockets, or load more code.

- **Hard limits:** a ~64 MB memory ceiling (an allocation past it aborts the script) and a wall-clock time budget of a few seconds (a runaway loop is killed). Either limit, any Lua compile/runtime error, or a missing/ill-typed handler surfaces as the command failing — GitCat reports the error.

- **Each invocation gets a fresh VM;** no state leaks between runs.

- **The sandbox constrains what a script can reach,** but git() can still read and write the repo — the same trust boundary as a shell run. Install a scripted plugin only if you'd run its code yourself.

### Mutating handlers must set mutates

Just like a shell command, a Luau handler that changes the repository (any git() call that writes — commit, reset, checkout, restore, …) must set `"mutates": true` on its command/hook. GitCat then takes a safety snapshot before running it, so the change is covered by global Undo (and a mutating command that can't be snapshotted is refused rather than run unprotected). GitCat can't infer this from your script, so always declare it — a mutating handler that omits mutates runs outside Undo. See Mutating actions & Undo.
...
```json
{
  ...
  "tama": {
    ...
    "voicePitch": 1.05,
    "copy": { "greeting": "Hi, I'm filling in for Tama today." }
  }
}
```
...
- Click Install plugin… and pick the plugin's plugin.json file (open the plugin's folder and select its plugin.json).

- The plugin appears in the list, enabled by default. Its commands are immediately available in ⌘K.
