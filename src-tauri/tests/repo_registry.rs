...
fn add_then_read_back_persists_across_a_simulated_restart() {
    ...
    repos.push(TrackedRepo { path: "/tmp/some/repo".into(), last_opened_at: None });
...
fn remove_then_read_back_persists_across_a_simulated_restart() {
    ...
    save_to(
        &file,
        &[
            TrackedRepo { path: "/tmp/repo-a".into(), last_opened_at: Some(100) },
            TrackedRepo { path: "/tmp/repo-b".into(), last_opened_at: Some(200) },
        ],
    )
    .unwrap();
...
fn track_opened_upserts_new_and_bumps_existing() {
    ...
    match repos.iter_mut().find(|r| r.path == "/tmp/repo-c") {
        ...
        None => repos.push(TrackedRepo { path: "/tmp/repo-c".into(), last_opened_at: Some(now1) }),
    }
    save_to(&file, &repos).unwrap();
    ...
    match repos.iter_mut().find(|r| r.path == "/tmp/repo-c") {
        ...
        None => repos.push(TrackedRepo { path: "/tmp/repo-c".into(), last_opened_at: Some(now2) }),
    }
    save_to(&file, &repos).unwrap();
...
fn a_path_that_is_not_a_git_repo_can_still_be_tracked_and_removed() {
    ...
    let mut repos = load_from(&file).unwrap();
    repos.push(TrackedRepo { path: normalize(bogus), last_opened_at: None });
    save_to(&file, &repos).unwrap();
