...
// Custom-item labels are supplied by the frontend (the single source of
// translations — see src/i18n + `syncNativeMenu` in legacy/main.ts), keyed by
// menu-item id, with submenu titles under `sub.*`. `labels` is empty for the
// English default menu Rust builds at startup (before the frontend has pushed
// the current locale), so every lookup falls back to its English default. The
// predefined items (Cut/Copy/Paste/Quit/Minimize/…) are localized by the OS,
// so they're never in this map.
pub fn build(app: &AppHandle<Wry>, labels: &HashMap<String, String>) -> tauri::Result<Menu<Wry>> {
    let lb = |key: &str, default: &'static str| -> String {
    ...
    let repo_menu = {
        ...
        let fetch = MenuItemBuilder::with_id("fetch", lb("fetch", "Fetch")).build(app)?;
        let pull = MenuItemBuilder::with_id("pull", lb("pull", "Pull")).build(app)?;
        let push = MenuItemBuilder::with_id("push", lb("push", "Push")).build(app)?;
        ...
        let refresh = MenuItemBuilder::with_id("refresh", lb("refresh", "Refresh")).build(app)?;
        SubmenuBuilder::new(app, lb("sub.repository", "Repository"))
            .item(&fetch)
            .item(&pull)
            .item(&push)
            .separator()
    ...
    let edit_menu = SubmenuBuilder::new(app, lb("sub.edit", "Edit")).cut().copy().paste().separator().select_all().build()?;

    let view_menu = {
        let toggle_theme = MenuItemBuilder::with_id("toggle-theme", lb("toggle-theme", "Toggle Theme")).build(app)?;
        ...
        let cmdk = MenuItemBuilder::with_id("cmdk", lb("cmdk", "Command Palette\u{2026}")).build(app)?;
        SubmenuBuilder::new(app, lb("sub.view", "View")).item(&toggle_theme).item(&cmdk).build()?
    };
    ...
    let tools_menu = {
        ...
        let history_menu = {
            let bisect = MenuItemBuilder::with_id("bisect", lb("bisect", "Bisect\u{2026}")).build(app)?;
            let reflog = MenuItemBuilder::with_id("reflog", lb("reflog", "Reflog\u{2026}")).build(app)?;
            let rerere = MenuItemBuilder::with_id("rerere", lb("rerere", "Rerere\u{2026}")).build(app)?;
            let plumbing = MenuItemBuilder::with_id("plumbing", lb("plumbing", "Plumbing\u{2026}")).build(app)?;
            ...
            // entry point.
            let repo_summary = MenuItemBuilder::with_id("repo-summary", lb("repo-summary", "Repository Summary\u{2026}")).build(app)?;
            ...
            // branch, …).
            let dangling_recovery = MenuItemBuilder::with_id("dangling-recovery", lb("dangling-recovery", "Dangling Commits\u{2026}")).build(app)?;
            SubmenuBuilder::new(app, lb("sub.history", "History"))
                .item(&bisect)
                .item(&reflog)
                .item(&rerere)
                .item(&plumbing)
        ...
        // repo-global.
        let patches_menu = {
            let export_patches = MenuItemBuilder::with_id("export-patches", lb("export-patches", "Export Patches\u{2026}")).build(app)?;
            let apply_patch = MenuItemBuilder::with_id("apply-patch", lb("apply-patch", "Apply Patch\u{2026}")).build(app)?;
            SubmenuBuilder::new(app, lb("sub.patches", "Patches")).item(&export_patches).item(&apply_patch).build()?
        };

        let remotes = MenuItemBuilder::with_id("remotes", lb("remotes", "Manage Remotes\u{2026}")).build(app)?;
        ...
        let repositories = MenuItemBuilder::with_id("repositories", lb("repositories", "Repositories\u{2026}")).build(app)?;
        ...
        let external_tools = MenuItemBuilder::with_id("external-tools", lb("external-tools", "External Tools\u{2026}")).build(app)?;
        ...
        let plugins = MenuItemBuilder::with_id("plugins", lb("plugins", "Plugins\u{2026}")).build(app)?;
        ...
        let repo_files = MenuItemBuilder::with_id("repo-files", lb("repo-files", "Repo Files (.gitignore / .mailmap)\u{2026}")).build(app)?;
        ...
        let uncommitted_changes = MenuItemBuilder::with_id("uncommitted-changes", lb("uncommitted-changes", "Uncommitted Changes")).build(app)?;
        let pull_merge = MenuItemBuilder::with_id("pull-merge", lb("pull-merge", "Pull (Merge)")).build(app)?;
        let pull_rebase = MenuItemBuilder::with_id("pull-rebase", lb("pull-rebase", "Pull (Rebase)")).build(app)?;
        ...
        let force_push_lease = MenuItemBuilder::with_id("force-push-lease", lb("force-push-lease", "Force Push (Safe)")).build(app)?;
        let force_push_override =
            MenuItemBuilder::with_id("force-push-override", lb("force-push-override", "Force Push (Override Remote)")).build(app)?;
        ...
        let reset_head = MenuItemBuilder::with_id("reset-head", lb("reset-head", "Reset HEAD to Commit\u{2026}")).build(app)?;
        ...
        let filter_repo = MenuItemBuilder::with_id("filter-repo", lb("filter-repo", "Rewrite History (filter-repo)\u{2026}")).build(app)?;
        SubmenuBuilder::new(app, lb("sub.tools", "Tools"))
            .item(&search_menu)
            .item(&history_menu)
            .item(&patches_menu)
            .item(&remotes)
    ...
    let help_menu = {
        let github = MenuItemBuilder::with_id("open-github", lb("open-github", "GitCat on GitHub")).build(app)?;
        let issue = MenuItemBuilder::with_id("report-issue", lb("report-issue", "Report an Issue\u{2026}")).build(app)?;
        ...
        let check_updates = MenuItemBuilder::with_id("check-for-updates", lb("check-for-updates", "Check for Updates\u{2026}")).build(app)?;
        let b = SubmenuBuilder::new(app, lb("sub.help", "Help")).item(&github).item(&issue).separator().item(&check_updates);
...
#[tauri::command]
#[specta::specta]
pub fn set_app_menu(app: AppHandle<Wry>, labels: HashMap<String, String>) -> Result<(), String> {
    let app_main = app.clone();
    app.run_on_main_thread(move || {
        if let Ok(menu) = build(&app_main, &labels) {
